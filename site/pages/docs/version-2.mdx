...
Instead of CSS keyframe animations, you can use TailwindCSS classes by wrapping it in the [Transition](https://headlessui.dev/voby/transition) component from [@headlessui/voby](https://headlessui.dev/).
...
You can now use the [`<Toaster/>`](/docs/toaster#using-a-custom-render-function) to render your own components. Pass in a function that receives a [Toast](https://github.com/timolins/voby-toast/blob/main/src/core/types.ts#L34) as the first argument, allowing you to render whatever you please.
...
```jsx
import { toast, Toaster, ToastBar } from 'voby-toast';
...
In the next releases, I plan to add the [most requested feature](https://github.com/timolins/voby-toast/issues/7): a dismiss button. As well as support for [custom toast types](https://github.com/timolins/voby-toast/issues/23).
