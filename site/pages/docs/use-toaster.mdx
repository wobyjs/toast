...
You can import only the core of the library with `voby-toast/headless`. It won't include any styles, dependencies or custom components.

```jsx
import { useToaster } from 'woby-toast/headless';
```

Be aware: [voby-toast 2.0](/docs/version-2) adds support for **custom render functions**, an easier method to render custom notification components.
...
Headless mode is perfectly suited to add notifications to your React Native app. You can check out [this example](<https://snack.expo.io/@timo/woby-toast---usetoaster()---voby-native>).
...
```jsx
import toast, { useToaster } from 'woby-toast/headless';
...
You can play with the demo on [CodeSandbox](https://codesandbox.io/s/woby-toast-usetoaster-headless-example-zw7op?file=/src/App.js).
